...
HRESULT CompileFormString( string code,
    ...
    hr = D3DCompile( code.c_str(), code.size(), NULL, pDefines, D3D_COMPILE_STANDARD_FILE_INCLUDE, pEntrypoint, pTarget, Flags1, Flags2, ppCode, &pErrorBlob );
...
public:
    ...
    struct TileConstBuffer{
        XMFLOAT4                    tilePos;
        XMFLOAT2                    mouseUV_clicked;
        XMFLOAT2                    tileReso;
        TileConstBuffer(){
            mouseUV_clicked = XMFLOAT2( 0.5f, 0.5f );
        }
    };
    ...
    D3D11_VIEWPORT                  m_RTviewportBackBuffer;
    ...
    TileConstBuffer*                m_pTileConstBuffer;
    ID3D11Buffer*                   m_pCBTileConstBuffer;
    ...
    void ComputeTileLocation( int iScreenWidth, int iScreenHeight ){
        ...
        for ( int i = 0; i < m_vecTiledObjs.size(); i++ ){
            ...
            m_pTileConstBuffer[i].tilePos = XMFLOAT4( tileCenterPos_x - halfTileOffset_x,
    ...
    string GenerateShaderCode()
    {
        ...
        shaderCode << "struct TileConstBuffer{\n";
        shaderCode << "    float4    TilePos;\n";
        shaderCode << "    float2    MousePos_Clicked;\n";
        shaderCode << "    float2    TileReso;\n";
        shaderCode << "};\n";
        shaderCode << "cbuffer cbTileLocation : register(b0)\n";
        shaderCode << "{\n TileConstBuffer TileCBs[" << m_vecTiledObjs.size() << "];\n }; \n";
        ...
        shaderCode << "float4 returnCol( float4 Col, float2 uv, uint PrimID ){\n";
        shaderCode << "    float distToMouse = length(( uv - TileCBs[PrimID].MousePos_Clicked )*TileCBs[PrimID].TileReso);\n";
        shaderCode << "    float4 Color = Col;\n";
        shaderCode << "    if ( distToMouse.x < 5.f ) Color.rgb = 1.f - Color.rgb;\n";
        ...
        shaderCode << "   output.Pos=float4(TileCBs[primID].TilePos.x,TileCBs[primID].TilePos.y,0.1f,1.f);\n";
        shaderCode << "   output.Tex=float2(0.f,0.f);\n";
        shaderCode << "   output.PrimID=primID;\n";
        shaderCode << "   triStream.Append(output);\n";
        shaderCode << "   output.Pos=float4(TileCBs[primID].TilePos.z,TileCBs[primID].TilePos.y,0.1f,1.f);\n";
        shaderCode << "   output.Tex=float2(1.f,0.f);\n";
        shaderCode << "   output.PrimID=primID;\n";
        shaderCode << "   triStream.Append(output);\n";
        shaderCode << "   output.Pos=float4(TileCBs[primID].TilePos.x,TileCBs[primID].TilePos.w,0.1f,1.f);\n";
        shaderCode << "   output.Tex=float2(0.f,1.f);\n";
        shaderCode << "   output.PrimID=primID;\n";
        shaderCode << "   triStream.Append(output);\n";
        shaderCode << "   output.Pos=float4(TileCBs[primID].TilePos.z,TileCBs[primID].TilePos.w,0.1f,1.f);\n";
        ...
        for ( int i = 0; i < m_vecTiledObjs.size(); i++ ){
            ...
            shaderCode << "case " << i << ":\n{\n";
            shaderCode << m_vecTiledObjs[i].strPScode << "return returnCol(color,input.Tex,input.PrimID);\n}\n";
        }
        shaderCode << "default:\n";
    ...
    TiledTextures( bool bRenderToBackbuffer = true )
    {
        m_bDirectToBackBuffer = bRenderToBackbuffer;

        m_uTextureWidth = 1280;
        m_uTextureHeight = 600;
                     ...
                     function<LRESULT( HWND, UINT, WPARAM, LPARAM )> _msgFunc = nullptr )
    {
        ...
        if ( strFormat.compare( 1, 5, "float" ) == 0 ){
            ...
            buff << "color = texture.Sample(samColor, input.Tex);\n";
            if ( !showMouseClickPos ) buff << "return color;\n";
        ...
        m_uTextureWidth = _iResWidth;
        m_uTextureHeight = _iResHeight;
    }
    ...
    HRESULT CreateResource( ID3D11Device* pd3dDevice )
    {
        ...
        m_pTileConstBuffer = new TileConstBuffer[m_vecTiledObjs.size()];
        ...
        cbDesc.MiscFlags = 0;
        cbDesc.ByteWidth = ( UINT ) m_vecTiledObjs.size() * sizeof( TileConstBuffer );
        V_RETURN( pd3dDevice->CreateBuffer( &cbDesc, NULL, &m_pCBTileConstBuffer ) );
    ...
    void SetupPipeline( ID3D11DeviceContext* pd3dImmediateContext )
    {
        ...
        pd3dImmediateContext->GSSetConstantBuffers( 0, 1, &m_pCBTileConstBuffer );
        pd3dImmediateContext->PSSetConstantBuffers( 0, 1, &m_pCBTileConstBuffer );
    ...
    void Resize( ID3D11Device* pd3dDevice, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc )
    {
        ...
        m_RTviewportBackBuffer.Width = ( float ) winWidth;
        m_RTviewportBackBuffer.Height = ( float ) winHeight;
        m_RTviewportBackBuffer.MinDepth = 0.0f;
        m_RTviewportBackBuffer.MaxDepth = 1.0f;
        m_RTviewportBackBuffer.TopLeftX = 0;
        m_RTviewportBackBuffer.TopLeftY = 0;
        ...
        for ( int i = 0; i < m_vecTiledObjs.size(); i++ ){
            ...
            m_pTileConstBuffer[i].tileReso.x = ( float ) subWidth;
            m_pTileConstBuffer[i].tileReso.y = ( float ) subHeight;
        }

        // Updating constant buffer
        ID3D11DeviceContext* pd3dImmediateContext = DXUTGetD3D11DeviceContext();
        pd3dImmediateContext->UpdateSubresource( m_pCBTileConstBuffer, 0, NULL, m_pTileConstBuffer, 0, 0 );
    ...
    void Render( ID3D11DeviceContext* pd3dImmediateContext )
    {
        ...
        pd3dImmediateContext->RSSetViewports( 1, &m_RTviewportBackBuffer );
    }
    ...
    void Destory()
    {
        ...
        SAFE_RELEASE( m_pSS );
        SAFE_RELEASE( m_pCBTileConstBuffer );
        if ( !m_bDirectToBackBuffer )
        {
        ...
        m_vecTiledObjs.clear();
        delete[] m_ppInputSRVs;
        delete[] m_ppNullSRVs;
        delete[] m_pTileConstBuffer;
    }

    LRESULT HandleMessages( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
    {
        LONG curse_x, curse_y;
        POINT cursorPos;
        GetCursorPos( &cursorPos );
        ScreenToClient( hWnd, &cursorPos );
        curse_x = cursorPos.x;
        curse_y = cursorPos.y;
        if ( uMsg == WM_LBUTTONDOWN ) m_bMouseHold = true;
        if ( uMsg == WM_LBUTTONUP ) m_bMouseHold = false;
        ...
        for ( int i = 0; i < m_vecTiledObjs.size(); i++ ){
            if ( curse_x > m_vecTiledObjs[i].iLTcorner_x &&
                ...
                LONG new_x = curse_x - m_vecTiledObjs[i].iLTcorner_x;
                LONG new_y = curse_y - m_vecTiledObjs[i].iLTcorner_y;
                if ( uMsg == WM_LBUTTONDOWN || ( uMsg == WM_MOUSEMOVE && m_bMouseHold ) ){
                    float mouseUV_x = new_x / m_pTileConstBuffer->tileReso.x;
                    float mouseUV_y = new_y / m_pTileConstBuffer->tileReso.y;
                    m_pTileConstBuffer[i].mouseUV_clicked = XMFLOAT2( mouseUV_x, mouseUV_y );
                }
