...
HRESULT Initial()
{
    ...
    UI.AddSlider( IDC_WINDHEADING_SLIDER, 0, iY += 26, 170, 23, 0, 360, ( int ) ( SimCloth.m_fWindHeading * 360.f / 6.2832f ) );

    swprintf_s( sz, 100, L"Wind Strength: %0.2f", SimCloth.m_fWindStrength );
    UI.AddStatic( IDC_WINDSTRENGTH_STATIC, sz, 0, iY += 26, 170, 23 );
    UI.AddSlider( IDC_WINDSTRENGTH_SLIDER, 0, iY += 26, 170, 23, 0, 50, ( int ) ( SimCloth.m_fWindStrength*2.0f ) );

    V_RETURN( MultiTexture.Initial() );
    MultiTexture.AddTexture( &SimCloth.m_pOutputTextureSRV, SUB_TEXTUREWIDTH, SUB_TEXTUREHEIGHT, false, "", "<float4>",
                             std::bind( &Cloth::Resize, &SimCloth, _1, _2, _3 ),
                             std::bind( &Cloth::HandleMessages, &SimCloth, _1, _2, _3, _4 ) );
    ...
    case IDC_WINDHEADING_SLIDER:
        {
            ...
            SimCloth.m_fWindHeading = k*3.14159f / 180.0f;
