...
public:
    ...
    UINT							m_uNumVertices;
    UINT							m_uNumAnchor;
    UINT							m_uNumRenderIndices;


    Particle*						m_pInitParticlePos;
    XMUINT2*						m_pAnchorPoint;
           ...
           UINT gridwidth = 161, UINT gridheight = 121, float scale = 10 )
    {
        ...
        m_uGridHeight = gridheight;
        m_uGridWidth = gridwidth;
        m_uNumVertices = gridheight * gridwidth;
        m_uNumAnchor = 5;
        ...
        m_fDt = 0.001f;
        m_fAccumulator = 0.f;
        m_fThresholdTime = 0.01f;
        m_fK_angleFactor = 0.1f;
        m_fWindHeading = 6.2f;
        m_fWindStrength = 15.f;

        m_pInitParticlePos = new Particle[m_uNumVertices];
        m_pAnchorPoint = new XMUINT2[m_uNumAnchor];
        /*m_pAnchorPoint[0] = int2( 0, 0 );
        m_pAnchorPoint[1] = int2( m_uGridWidth / 2 , 0 );
        m_pAnchorPoint[2] = int2((m_uGridWidth - 1), 0);
        */
        int intival = m_uGridHeight / ( m_uNumAnchor - 1 );
        for ( UINT i = 0; i < m_uNumAnchor; ++i ){
            UINT y = i*intival;
            if ( y >= m_uGridHeight ) y = m_uGridHeight - 1;
            m_pAnchorPoint[i] = XMUINT2( 0, y );
        }

        m_CBperCall.iWidth = gridwidth;
        m_CBperCall.iHeight = gridheight;
        m_CBperCall.fK_length = 300000.f;
        m_CBperCall.fK_angle = m_CBperCall.fK_length * m_fK_angleFactor;
        m_CBperCall.fB = 1;
        m_CBperCall.vG = float3( 0.f, -9.8f, 0.f );
        m_CBperCall.fL0 = 2.0f*scale / static_cast< float >( m_uGridWidth - 1 );
        m_CBperCall.fKair = 90.f;
        m_CBperCall.fInvM = 1.0f / 1.5f;

        m_CBperCall.fWindVel = float3( cos( m_fWindHeading )*m_fWindStrength, 0, sin( m_fWindHeading )*m_fWindStrength );

        m_CBperCall.vSpherePos = float3( -0.6f*scale, 0.3f*scale, 0.f );
        m_CBperCall.fSphereRadius = 0.3f*scale;
    }
    ...
    {
        HRESULT hr;
        float* texArray = new float[4 * m_uGridHeight*m_uGridWidth];
        for ( UINT y = 0; y < m_uGridHeight; ++y ){
            for ( UINT x = 0; x < m_uGridWidth; ++x ){
                float3	pos;
                pos.x = 2.0f * x / static_cast< float >( m_uGridWidth - 1 );
                pos.y = 2.0f * y / static_cast< float >( m_uGridWidth - 1 ) - 0.3f;
                pos.z = 0.f;

                texArray[m_uGridWidth * 4 * y + x * 4 + 0] = pos.x*m_fClothScale;
                texArray[m_uGridWidth * 4 * y + x * 4 + 1] = pos.y*m_fClothScale;
                texArray[m_uGridWidth * 4 * y + x * 4 + 2] = pos.z*m_fClothScale;
                texArray[m_uGridWidth * 4 * y + x * 4 + 3] = FREE_PARTICLE;
                for ( UINT i = 0; i < m_uNumAnchor; ++i ){
                    if ( m_pAnchorPoint[i].x == x && m_pAnchorPoint[i].y == y ){
                        texArray[m_uGridWidth * 4 * y + x * 4 + 3] = ANCHOR_PARTICLE;
                        //texArray[m_uGridWidth * 4 * y + x * 4 + 1] *= 0.8;
                    }
                }
            }
        }
        ...
        initialData.SysMemPitch = m_uGridWidth * 4 * sizeof( float );
        initialData.SysMemSlicePitch = m_uGridHeight * m_uGridWidth * 4 * sizeof( float );


        D3D11_TEXTURE2D_DESC dstex;
        ZeroMemory( &dstex, sizeof( D3D11_TEXTURE2D_DESC ) );
        dstex.Width = m_uGridWidth;
        dstex.Height = m_uGridHeight;
        dstex.MipLevels = 1;
    ...
    HRESULT CreateClothVelTex( ID3D11Device* pd3dDeivce )
    {
        ...
        for ( UINT y = 0; y < m_uGridHeight; ++y ){
            for ( UINT x = 0; x < m_uGridWidth; ++x ){
                texArray[m_uGridWidth * 4 * y + x * 4 + 0] = 0;
                texArray[m_uGridWidth * 4 * y + x * 4 + 1] = 0;
                texArray[m_uGridWidth * 4 * y + x * 4 + 2] = 0;
                texArray[m_uGridWidth * 4 * y + x * 4 + 3] = 1;
            }
        }
    ...
    HRESULT CreateClothPosVB( ID3D11Device* pd3dDevice )
    {
        ...
        for ( UINT y = 0; y < m_uGridHeight; ++y ){
            for ( UINT x = 0; x < m_uGridWidth; ++x ){
                float3	pos;
                pos.x = 2.f * x / static_cast< float >( m_uGridWidth - 1 ) - 1.f;
                pos.y = 2.f * y / static_cast< float >( m_uGridHeight - 1 ) - 1.f;
                pos.z = 0;
        ...
        {
            m_uNumVertices * sizeof( Particle ),
    ...
    HRESULT CreateClothRenderIB( ID3D11Device* pd3dDevice )
    {
        ...
        m_uNumRenderIndices = 2 * ( m_uGridHeight - 1 ) * ( m_uGridWidth - 1 ) * 3;
        int size = m_uNumRenderIndices;
        ...
        int n = 0;
        for ( UINT x0 = 0; x0 < m_uGridWidth - 1; x0 += subWidth ){
            UINT x1 = x0 + subWidth;
            for ( UINT y = 0; y < m_uGridHeight - 1; ++y ){
                for ( UINT x = x0; ( x < m_uGridWidth - 1 ) && ( x < x1 ); ++x ){
                    indices[n][0] =(WORD)(( y + 1 ) * m_uGridWidth + ( x + 0 ));
                    indices[n][1] =(WORD)(( y + 0 ) * m_uGridWidth + ( x + 0 ));
                    indices[n][2] =(WORD)(( y + 1 ) * m_uGridWidth + ( x + 1 ));
                    ++n;
                    indices[n][0] =(WORD)(( y + 0 ) * m_uGridWidth + ( x + 0 ));
                    indices[n][1] =(WORD)(( y + 0 ) * m_uGridWidth + ( x + 1 ));
                    indices[n][2] =(WORD)(( y + 1 ) * m_uGridWidth + ( x + 1 ));
                    ++n;
                }
            }
    ...
    HRESULT CreateResource( ID3D11Device* pd3dDevice )
    {
        ...
        m_SimViewport.Width = ( float ) m_uGridWidth;
        m_SimViewport.Height = ( float ) m_uGridHeight;
        m_SimViewport.MinDepth = 0.0f;
        m_SimViewport.MaxDepth = 1.0f;
        m_SimViewport.TopLeftX = 0;
        m_SimViewport.TopLeftY = 0;

        XMVECTORF32 vecEye = { 0.0f, 4.0f, -10.0f };
        XMVECTORF32 vecAt = { 0.0f, 4.0f, 0.0f };
    ...
    void Render( ID3D11DeviceContext* pd3dImmediateContext, double fTime, float fElapsedTime )
    {
        ...
        pd3dImmediateContext->DrawIndexed( m_uNumRenderIndices, 0, 0 );
