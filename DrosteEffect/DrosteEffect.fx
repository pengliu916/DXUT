...
float2 RepetitionSolver( float2 xy, inout int intRepetition )
{
    float2 r = xy;
    intRepetition = 0;
    for ( uint i = 0; i < 30; i++ )
    {
        ...
        float alpha = tex.SampleLevel( samGeneral, float3(ConvertToUV( resultXY, inputTexReso, center ),intRepetition), 0 ).a;
        if ( alpha >= 0.5f ) return r;
        else
        {
            r.x += log( separate / R1 );
            intRepetition++;
            if ( intRepetition >= 4.f )  intRepetition = 0.f;
            continue;
        }
    }
    return r;
...
float2 Droste( float2 uv, inout uint rep)
{
...
float4 PS( float4 Pos : SV_Position, float2 TexUV : TexCoord0 ) : SV_Target
{
    ...
    int rep = 0;
    xy = Droste( t*xy, rep );
    float4 col = tex.SampleLevel( samGeneral, float3(ConvertToUV( xy, inputTexReso, center ),rep), 0 );
    return col;
}
